...
class Discover extends Console\Command\Command
{
	...
	protected function execute(Input\InputInterface $input, Output\OutputInterface $output): int
	{
		...
		$io->note((string) $this->translator->translate('//shelly-connector.cmd.discover.subtitle'));
		...
		if (
			...
			if ($connector === null) {
				$io->warning(
					(string) $this->translator->translate(
						'//shelly-connector.cmd.discover.messages.connector.notFound',
					),
				);
			...
			if (count($connectors) === 0) {
				$io->warning(
					(string) $this->translator->translate('//shelly-connector.cmd.base.messages.noConnectors'),
				);
			...
			if (count($connectors) === 1) {
				...
				if ($input->getOption('no-interaction') === false) {
					$question = new Console\Question\ConfirmationQuestion(
						(string) $this->translator->translate(
							'//shelly-connector.cmd.discover.questions.execute',
				...
				$question = new Console\Question\ChoiceQuestion(
					(string) $this->translator->translate('//shelly-connector.cmd.discover.questions.select.connector'),
		...
		if (!$connector->isEnabled()) {
			$io->warning(
				(string) $this->translator->translate('//shelly-connector.cmd.discover.messages.connector.disabled'),
			);
		...
		$io->info((string) $this->translator->translate('//shelly-connector.cmd.discover.messages.stopping'));

		if ($result !== Console\Command\Command::SUCCESS) {
			$io->error((string) $this->translator->translate('//shelly-connector.cmd.execute.messages.error'));
	...
	): void
	{
		...
		$table->setHeaders([
			'#',
			(string) $this->translator->translate('//shelly-connector.cmd.discover.data.id'),
			(string) $this->translator->translate('//shelly-connector.cmd.discover.data.name'),
			(string) $this->translator->translate('//shelly-connector.cmd.discover.data.generation'),
			(string) $this->translator->translate('//shelly-connector.cmd.discover.data.model'),
			(string) $this->translator->translate('//shelly-connector.cmd.discover.data.address'),
		]);
		...
		if ($foundDevices > 0) {
			$io->info(sprintf(
				(string) $this->translator->translate('//shelly-connector.cmd.discover.messages.foundDevices'),
				$foundDevices,
			));
			...
			$io->info((string) $this->translator->translate('//shelly-connector.cmd.discover.messages.noDevicesFound'));
		}

		$io->success((string) $this->translator->translate('//shelly-connector.cmd.discover.messages.success'));
	}
